...
def setup(options):
    ...
    if central_IA_depends_on not in ['constant', 'luminosity', 'halo_mass']:
        raise ValueError('Choose one of the following options for central_IA_depends_on: constant, luminosity, halo_mass')

    if satellite_IA_depends_on not in ['constant', 'luminosity', 'halo_mass']:
        raise ValueError('Choose one of the following options for satellite_IA_depends_on: constant, luminosity, halo_mass')
    ...
    if central_IA_depends_on == 'luminosity':
        ...
        lum_centrals, lum_pdf_z_centrals = compute_luminosity_pdf(z_loglum_file_centrals, zmin, zmax, nz, nlbins)
    else:
        nlbins = 100000
    ...
    if satellite_IA_depends_on == 'luminosity':
        ...
        lum_satellites, lum_pdf_z_satellites = compute_luminosity_pdf(z_loglum_file_satellites, zmin, zmax, nz, nlbins)
    else:
        nlbins = 100000
...
def execute(block, config):
    ...
    if central_IA_depends_on == 'constant':
        ...
        if block.has_value(f'intrinsic_alignment_parameters{suffix}', 'beta_two'):
            ...
            mean_lscaling = np.array([broken_powerlaw(lum_centrals[i], lum_pdf_z_centrals[i], gamma_2h, 10.0**lpiv, beta, beta_two) for i in range(nz)])
        else:
            mean_lscaling = gamma_2h * mean_L_L0_to_beta(lum_centrals, lum_pdf_z_centrals, 10.0**lpiv, beta)
