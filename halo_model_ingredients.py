...
def concentration_colossus(block, cosmo, mass, z, model, mdef, overdensity):
    ...
    consistently with halo mass function and halo bias function.
    """
    with warnings.catch_warnings():
        warnings.filterwarnings('ignore', category=UserWarning)
        # This disables the warning from colossus that is just telling us what we know
        ...
        this_cosmo = colossus_cosmology.fromAstropy(
            astropy_cosmo=cosmo, cosmo_name='custom',
    ...
    # MA: Possible solution: See if we can get away with a smaller number of redshifts and interpolate.
    # tic = time.perf_counter()
    c, ms = colossus_concentration.concentration(
        M=mass, z=z, mdef=mdef.colossus_name, model=model,
...
def setup(options):
    ...
    # currently we have only implemented NFW.
    ...
    # most general astropy cosmology initialization,
    ...
    # This is the slow part it takes 1.58/1.67
    mf = MassFunction(
        z=0., cosmo_model=initialise_cosmo, Mmin=log_mass_min,
...
def execute(block, config):
    ...
    for jz, z_iter in enumerate(z_vec):
        if mdef in ['SOVirial'] and mead_correction is None:
            ...
            # Then it is faster because it only updates the redshift and the corresponding delta_c
            mf.update(
                z=z_iter, cosmo_model=this_cosmo_run, sigma_8=sigma_8, n=ns, delta_c=delta_c_z,
            ...
            with warnings.catch_warnings():
                ...
                mdef_mead = hmu.SOVirial_Mead  # 'SOMean' # Need to use SOMean to correctly parse the Mead overdensity as calculated above! Otherwise the code again uses the Bryan & Norman function!
                mdef_conc = mdef_mead
                mf.ERROR_ON_BAD_MDEF = False
                mf.update(
                    z=z_iter, cosmo_model=this_cosmo_run, sigma_8=sigma_8, n=ns, delta_c=delta_c_z,
                    mdef_model=mdef_mead, mdef_params={'overdensity': halo_overdensity_mean[jz]},
                    disable_mass_conversion=True, transfer_model='FromArray',
            ...
            mf.update(
                z=z_iter, cosmo_model=this_cosmo_run, sigma_8=sigma_8, n=ns, delta_c=delta_c_z,
                mdef_params={'overdensity': halo_overdensity_mean[jz]},
                transfer_model='FromArray', transfer_params={'k': transfer_k, 'T': transfer_func}
            )
            mdef_conc = mdef
        ...
        if hmf_code == 'CCL':
            ...
            cosmo_ccl = ccl.Cosmology(
                Omega_c=block[cosmo_params, 'omega_m'] - block[cosmo_params, 'omega_b'],
        ...
        # This is the redshift dependent mean density
    ...
    if mead_correction == 'feedback':
        ...
        eta_cen = (0.1281 * sigma8_z[:, np.newaxis] ** (-0.3644))
        conc = (1.0 + zf) / (1.0 + z_vec[:, np.newaxis])
    # TODO: what happens if mead_correction is stellar_fraction_from_observable_feedback?
    ...
    # k = np.logspace(-2,1,200) # AD: inherit the range from Plin? That would avoid interpolations ...
