...
def test_initialization(setup_data):
    ...
    spectra = UpsampledSpectra(z=z, k=k, fraction_z=fraction_z, fraction=fraction,
                               model_1_params=model_1_params, model_2_params=model_2_params)
...
def test_add_spectra(setup_data):
    ...
    expected_gm = spectra.frac_1[:, np.newaxis] * pk_1 + (1.0 - spectra.frac_1[:, np.newaxis]) * pk_2
    assert np.allclose(added_power_gm, expected_gm)

    added_power_other = spectra.add_spectra(pk_1, pk_2, 'other')
    expected_other = spectra.frac_1[:, np.newaxis]**2.0 * pk_1 + (1.0 - spectra.frac_1[:, np.newaxis])**2.0 * pk_2
