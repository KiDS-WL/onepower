...
def test_hod_initialization(setup_data):
    ...
    hod = HOD(cosmo=cosmo, mass=mass, dndlnm=dndlnm, halo_bias=halo_bias, z_vec=z_vec, hod_settings=hod_settings)
...
def test_cacciato_initialization(setup_data):
    mass, dndlnm, halo_bias, z_vec, cosmo, hod_settings = setup_data
    cacciato = Cacciato(cosmo=cosmo, mass=mass, dndlnm=dndlnm, halo_bias=halo_bias, z_vec=z_vec, hod_settings=hod_settings)
    assert isinstance(cacciato, Cacciato)

def test_simple_initialization(setup_data):
    mass, dndlnm, halo_bias, z_vec, cosmo, hod_settings = setup_data
    simple = Simple(cosmo=cosmo, mass=mass, dndlnm=dndlnm, halo_bias=halo_bias, z_vec=z_vec, hod_settings=hod_settings)
    assert isinstance(simple, Simple)

def test_zehavi_initialization(setup_data):
    mass, dndlnm, halo_bias, z_vec, cosmo, hod_settings = setup_data
    zehavi = Zehavi(cosmo=cosmo, mass=mass, dndlnm=dndlnm, halo_bias=halo_bias, z_vec=z_vec, hod_settings=hod_settings)
    assert isinstance(zehavi, Zehavi)

def test_zheng_initialization(setup_data):
    mass, dndlnm, halo_bias, z_vec, cosmo, hod_settings = setup_data
    zheng = Zheng(cosmo=cosmo, mass=mass, dndlnm=dndlnm, halo_bias=halo_bias, z_vec=z_vec, hod_settings=hod_settings)
    assert isinstance(zheng, Zheng)

def test_zhai_initialization(setup_data):
    mass, dndlnm, halo_bias, z_vec, cosmo, hod_settings = setup_data
    zhai = Zhai(cosmo=cosmo, mass=mass, dndlnm=dndlnm, halo_bias=halo_bias, z_vec=z_vec, hod_settings=hod_settings)
