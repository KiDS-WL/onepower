...
if __name__ == '__main__':
    ...
    df_obs = np.power(10., -0.4*(df['sdss_r_abs_mag'] - 4.76))
    ...
    for i in range(len(zbins)):
        selection = df_obs[(df['Z_B'] < edges[i]) & (df['Z_B'] >= edges[i-1]) & (df_ur > split_value) & (df['flag_central'] == 0)]
        obs_min[i] = np.min(selection)
        obs_max[i] = np.max(selection)
        
    np.savetxt('red_cen_lum_pdf.txt', np.column_stack([z_bins, obs_min, obs_max]), header='z lum_min lum_max')
    ...
    np.savetxt('blue_cen_lum_pdf.txt', np.column_stack([z_bins, obs_min, obs_max]), header='z lum_min lum_max')
    ...
    c1=fits.Column(name='z', array=df['Z_B'][(df_ur <= split_value) & (df['flag_central'] == 1)], format='E')
    c2=fits.Column(name='loglum', array=np.log10(df_obs[(df_ur <= split_value) & (df['flag_central'] == 1)]), format='E')
    ...
    c1=fits.Column(name='z', array=df['Z_B'][(df_ur > split_value) & (df['flag_central'] == 0)], format='E')
    c2=fits.Column(name='loglum', array=np.log10(df_obs[(df_ur > split_value) & (df['flag_central'] == 0)]), format='E')
    t=fits.BinTableHDU.from_columns([c1,c2])
