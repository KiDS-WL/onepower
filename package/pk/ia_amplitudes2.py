...
class AlignmentAmplitudes:
    def __init__(self,
        ...
        if mpivot_cen is not None:
            self.mpivot_cen = 10.0**mpivot_cen
            
        if mpivot_sat is not None:
            self.mpivot_sat = 10.0**mpivot_sat
            
        if lpivot_cen is not None:
            self.lpivot_cen = 10.0**lpivot_cen
            
        if lpivot_sat is not None:
            self.lpivot_sat = 10.0**lpivot_sat
    ...
    def _initialize_luminosity_arrays(self, galaxy_type):
        ...
        if galaxy_type == 'centrals':
            depends_on = self.central_ia_depends_on
            z_loglum_file = self.z_loglum_file_centrals
        else:
            depends_on = self.satellite_ia_depends_on
            z_loglum_file = self.z_loglum_file_satellites
    ...
    def _process_centrals(self):
        ...
        if self.central_ia_depends_on == 'constant':
            self.alignment_gi = self.gamma_2h_am * np.ones_like(self.z_vec)
