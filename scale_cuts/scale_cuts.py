...
def execute(block, config):
    ...
    if config["simulate"]:
        ...
        if config["simulate_with_noise"]:
            ...
            print('data mean vector',config["TP_data"].makeMeanVector())
            config["TP_data"].replaceMeanVector(mu)
            print("theory mean vector",mu)
            print('simulated mean vector',config["TP_data"].makeMeanVector())
            if config["mock_filename"] != "":
                config["TP_data"].to_fits(config["mock_filename"]+".fits", overwrite=True)
