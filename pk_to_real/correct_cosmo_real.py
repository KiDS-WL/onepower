...
def execute(block, config):
    ...
    for i in range(nbins):
        ...
        arr = block[section_name, f'rp']
        zi = z[i]
        
        ratio = ((cosmo_model_run.comoving_distance(zi)) / (cosmo_model_data.comoving_distance(zi))) * (h_run/h_data)
        if corr_type == 'wp':
            ratio_corr = (cosmo_model_data.efunc(zi) / cosmo_model_run.efunc(zi)) * (h_data/h_run)
        elif corr_type == 'ds':
            ratio_corr = ((cosmo_model_data.angular_diameter_distance(z_s) * cosmo_model_run.angular_diameter_distance_z1z2(zi, z_s) * cosmo_model_run.angular_diameter_distance(zi)) \
                      / (cosmo_model_run.angular_diameter_distance(z_s) * cosmo_model_data.angular_diameter_distance_z1z2(zi, z_s) * cosmo_model_data.angular_diameter_distance(zi))) * (h_run/h_data)
        else:
            raise ValueError('Unrecognised correlation type')
        
        func_new = func * ratio_corr
        arr_new = arr * ratio
        
        block.replace_double_array_1d(section_name, f'bin_{i+1}', func_new)
        block.replace_double_array_1d(section_name, f'rp', arr_new)
