...
os.environ['CUDA_VISIBLE_DEVICES'] = '-1'  # to use CPU if GPU is available otherwise the GPU memory runs out of memory. It also does not slow the prediction.
...
def get_linear_power_spectrum(block, z_vec):
    # AD: growth factor should be computed from camb/hmf directly, this way we can load Plin directly without this function!
...
def get_cosmopower_inputs(block, z_vec, len_zvec, len_Mvec, M_vec):
    # Get parameters from block and give them the names and form that class expects
    ...
    params_bnl = {
        'ombh2': ombh2 * np.ones_like(z_list),
